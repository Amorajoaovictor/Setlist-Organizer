...
async function getTrackBpmMap(tracks: DeezerTrack[]) {
  ...
  const missingBpmTracks = tracks.filter((track) => track.bpm == null);

  await Promise.all(
    missingBpmTracks.map(async (track) => {
...
async function fetchTrackBpm(trackId: string) {
  try {
    ...
    // Keep search usable even when a detail lookup fails or Deezer omits BPM.
    return null;
  }
}
